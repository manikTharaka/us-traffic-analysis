...
val_cols = [col for col in df.columns if col.startswith("traffic_volume_counted_after")]

# bystate = df.groupby('fips_state_code',as_index=False).sum()
bystate = df.melt(id_vars=['fips_state_code'],value_vars=val_cols)
del  df
gc.collect()

bystate.to_csv('data/state_sum.csv',index=False)